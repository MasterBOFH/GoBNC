...
<host> may be a bare hostname or a ws://host / wss://host[:port][/path] URL;
a wss:// (or ws://) host makes the uplink a WebSocket connection (wss = TLS,
ws = plaintext) and sets the upgrade path — equivalent to --websocket=true.
