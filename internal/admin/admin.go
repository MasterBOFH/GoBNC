...
func runNetwork(ctx context.Context, deps Deps, opts Options, args []string) ([]string, error) {
	...
	case "list":
		...
		for _, n := range nets {
			lines = append(lines, fmt.Sprintf("%s\t%s:%d\ttls=%v\ttls_noverify=%v\tnick=%s\talt_nick=%s\tnick_recovery=%v\tflood_burst=%d\tflood_rate=%g",
...
func networkAdd(ctx context.Context, deps Deps, opts Options, args []string) ([]string, error) {
	if len(args) < 4 {
		return nil, fmt.Errorf("usage: network add <name> <host> <port> [nick] [--nick=] [--tls=true] [--tls-noverify=true|false] [--user=] [--realname=] [--sasl-user=] [--sasl-pass] [--flood-burst=] [--flood-rate=] [--alt-nick=] [--nick-recovery=true|false]")
	}
	if deps.Runtime == nil {
		return nil, fmt.Errorf("runtime not configured")
	}
...
func networkMod(ctx context.Context, deps Deps, opts Options, args []string) ([]string, error) {
	if len(args) < 2 {
		return nil, fmt.Errorf("usage: network mod <name> [--host=] [--port=] [--nick=] [--tls=true|false] [--tls-noverify=true|false] [--user=] [--realname=] [--sasl-user=] [--sasl-pass] [--flood-burst=] [--flood-rate=] [--alt-nick=] [--nick-recovery=true|false]")
	}
	if deps.Runtime == nil {
		return nil, fmt.Errorf("runtime not configured")
	}
	...
	if !changed {
		return nil, fmt.Errorf("network mod: no changes; pass at least one --host/--port/--nick/--tls=/--tls-noverify=/--user=/--realname=/--sasl-*/--flood-*/--alt-nick=/--nick-recovery=")
	}
	if _, err := deps.Store.UpsertNetwork(ctx, n); err != nil {
		return nil, err
	}
	...
	if ok {
		return []string{fmt.Sprintf("updated %s (flood pacing applies now; host/TLS/SASL on next uplink reconnect)", n.Name)}, nil
	}
