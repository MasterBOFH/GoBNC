...
func TestCanUpgradeNormalizesUnversionedKeeper(t *testing.T) {
	...
	if KeeperVersion != 3 || MinKeeperVersion != 1 {
	...
	if got := CanUpgrade(0); got != UpgradeShould {
		t.Errorf("CanUpgrade(0)=%s, want should (unversioned keeper ≡ 1, older than current, still >= min 1)", got)
	}
	if got := CanUpgrade(2); got != UpgradeShould {
		t.Errorf("CanUpgrade(2)=%s, want should", got)
	}
	if got := CanUpgrade(3); got != UpgradeNone {
		t.Errorf("CanUpgrade(3)=%s, want none", got)
	}
	if WSMinKeeperVersion != 3 {
		t.Errorf("WSMinKeeperVersion=%d, want 3", WSMinKeeperVersion)
	}
}
